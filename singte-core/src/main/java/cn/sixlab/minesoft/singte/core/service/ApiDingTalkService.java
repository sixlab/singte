...
public class ApiDingTalkService extends BaseController {
    ...
    public void listTask(String dingUserId, StUser stUser) {
        String task = dingTalkJob.listAllTask(stUser);
        dingTalkService.sendSampleText(dingUserId, task);

        listTodo(dingUserId, stUser);
    }
    ...
    public void delete(String dingUserId, StUser stUser, Integer indexNo) {
        ...
        if (null != stTodo) {
            ...
            String sb = "编号[" + indexNo + "]任务已删除：" + stTodo.getTodoName() +
                    "\n\n" + taskMessage +
                    "\n\n" + todoMessage;
